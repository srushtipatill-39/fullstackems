...
const autoCheckOut = inngest.createFunction(
    ...
    async ({ event, step }) => {
        const { employeeId, attendeanceId } = event.data;

        //Wait for 9 hours
        await step.sleepUntil("wait-for-the-9-hours", new Date(new Date
            ().getTime() + 9 * 60 * 60 * 1000
        ))

        //get Attendance data
        let attendance = await Attendance.findById(attendeanceId)
      ...
      <p style="font-size: 16px;">EMS</p>
            ...
            attendance = await Attendance.findById(attendeanceId)
            if (!attendance?.checkOut) {
...
// Send email to Admin, If admin doesn't take action pon leave application within 24 hours
const leaveApplicationReminder = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        //wait for 24 hours
        await step.sleepUntil("wait-for-the-24-hour", new Date(new Date().getTime() + 24 * 60 * 60 * 1000))
...
const attendanceReminderCron = inngest.createFunction(
    ...
    async ({ step }) => {
        ...
        //Step 4: Get employee IDs who alredy checked in today
